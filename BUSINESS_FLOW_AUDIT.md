...
| W7 | Follow-up management | Doctor tracks follow-ups from consultations | Follow-ups page lists pending with dates; status update unrestricted | **PARTIAL** (needs confirmation) |
...
| B7 | LOW | Bill edit silently rewrites linked income transaction | Payment history amount changed after the fact; no snapshot | `updateBill` overwrites linked transaction with new `receivedAmount` | Doctor / Billing | History rewritten | Preserve original payment events | Not changed — **requires product decision** | — |
...
| B9 | LOW | UTC date slicing vs IST | "Today" boundaries off by a day between 00:00–05:30 IST | `toISOString().slice(0,10)` used for `today` in KPI/booking checks | All | Edge-case misclassification | Server-local date | Not changed — low impact; **requires confirmation** | — |
